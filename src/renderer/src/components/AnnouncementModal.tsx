    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-start gap-4 p-6 border-b border-gray-200">
          <div className={`flex-shrink-0 p-3 rounded-full ${colors.iconBg} ${colors.iconColor}`}>
            {getIcon()}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className={`p-4 rounded-xl ${colors.bg} border ${colors.border}`}>
            <div className={`text-sm leading-relaxed whitespace-pre-wrap ${colors.text}`}>
        ...
        <div className="flex gap-3 p-6 border-t border-gray-200">
          ...
          {announcement.dismissible && (
            <button
              onClick={handleDismiss}
              className={`flex-1 px-4 py-2.5 text-white font-medium rounded-lg transition-colors ${colors.button}`}
            >
              我知道了
            </button>
          )}
