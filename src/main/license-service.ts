...
function getServerUrl(): string {
  return getAppConfig().license.serverUrl
}

function getPublicKeyB64(): string {
  return getAppConfig().license.publicKey
}
...
export class LicenseService {
  ...
  async deactivate(): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      logger.warn('解绑请求失败，但仍然清除本地数据:', e)
    }
    // 清除所有许可证相关的配置
